...
@tool
@icon("icon.png")
class_name ByteBrew extends Node

signal remote_config_updated(status: bool)
signal purchase_result_received()
...
func _update_plugin() -> void:
	if _plugin_singleton == null:
		if Engine.has_singleton(PLUGIN_SINGLETON_NAME):
			...
			ByteBrew.log_error("%s singleton not found on this platform!" % PLUGIN_SINGLETON_NAME)
