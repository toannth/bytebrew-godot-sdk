...
There are 2 ways to install the `ByteBrew` plugin into your project:
...
- connect `ByteBrew` node signals
	- `template_ready(a_template: ByteBrewInfo)`
	- ...

Example usage:
```
@onready var byte_brew := $ByteBrew

func _ready():
	byte_brew.template_ready.connect(_on_template_ready)

	var templates: Array[ByteBrewInfo] = byte_brew.get_byte_brew()
	for template in templates:
		print("Template description: %s" % [template.get_description()])

func _on_template_ready(template: ByteBrewInfo):
